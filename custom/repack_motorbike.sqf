if (dayz_combat == 1) then { 
    cutText [format["You are in Combat and cannot repack your motorcycle"], "PLAIN DOWN"];
} else {
	_startDeploy = time;
	...
	if (_finished) then {
		...
		player switchMove "";
		player playActionNow "stop";
		cutText [format["Motorcycle packing interrupted!"], "PLAIN DOWN"]; //display text at bottom center of screen on interrupt
	};
};